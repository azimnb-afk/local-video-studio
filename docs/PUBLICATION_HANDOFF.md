...
1. [ ] **Model Terms Review**: Spot-check Hugging Face model cards for [Lightricks/LTX-2.3](https://huggingface.co/Lightricks/LTX-2.3) and [Google Gemma](https://ai.google.dev/gemma/terms).
...
Create a new public repository on GitHub (e.g. named `local-video-studio` or `local-video-studio-mac`).  
...
5. Attach the source release archive `Local-Video-Studio-v0.9.0-preview.1-source.zip` (and `.sha256`).
