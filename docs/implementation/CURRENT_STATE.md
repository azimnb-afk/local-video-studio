...
## 2026-08-09 CharacterBible Phase 2 — Derived Reference Assets
- `CharacterReferenceAsset` now records optional derived provenance (`sourceAssetID`, normalized top-left `sourceCropRect`, original dimensions, extraction method, analysis provider/model, and user-adjusted state) without breaking Phase 0/1 project decoding. The original Character Sheet remains the source of truth.
- `CharacterSheetRegionAnalyzer` asks the installed local Vision provider for schema-constrained Front / Side / Back / Face / Expression / Costume Detail regions. Provider coordinates are validated and immediately normalized to `x/y/width/height` in `0...1`, origin top-left. Invalid, zero-size, out-of-bounds, or impractically small crops are rejected; only tiny floating overflow is clamped.
- `CharacterReferenceExtractionService` maps reviewed normalized coordinates to the project-owned ORIGINAL image, normalizes EXIF orientation, writes lossless PNGs without filtering/upscaling, and atomically adds files under `Assets/Characters/<CharacterID>/References/<UUID>.png`. Batch failure cleans staged files and never leaves metadata-only references.
- Storyboard and Hybrid share one Character Editor flow: best-effort auto proposals, visible overlays/previews, move/resize and numeric correction, type/label correction, proposal removal/uncheck, manual region addition, local thumbnails, and derived-asset deletion. Cancel creates neither files nor metadata. Deleting a source sheet does not delete already materialized derived PNGs; missing provenance is tolerated on reload.
- Actual 1086×1448 acceptance sheet `/Users/azimnb/Downloads/ChatGPT Image 2026年8月9日 15_00_18.png` produced five auto proposals with `agents-a1:32k`. Side/Back were near useful; Front, Expression, and Costume Detail required correction, and Face was added manually. Localization classification: **C — semantic detection succeeds, but bounding boxes are unstable and mandatory user review is required**.
- User-reviewed saved assets: Front 287×774, Side 174×751, Back 272×769, Expression 191×228, Costume Detail 145×202, and Close-Up 344×499. All are project-owned PNGs with source UUID/crop provenance. Source and managed-sheet SHA-256 remained identical (`1b7c272b...23f89`). Quit/relaunch restored every thumbnail and the same Character/Shot UUID relationship.
