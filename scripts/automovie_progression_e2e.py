...
def main():
    ...
    for i, shot in enumerate(shots):
        ...
        # Opening image for shot 1. The shipping pipeline takes this from the
        # project's Opening Reference Image, or failing that its Character
        # Anchor; both condition the first frame at 1.0. Mirrored here by env so
        # a run with an opening image can be compared against a text-to-video
        # one. LTX_E2E_CHARACTER_ANCHOR is kept as the older spelling.
        opening = (os.environ.get("LTX_E2E_OPENING_REFERENCE")
                   or os.environ.get("LTX_E2E_CHARACTER_ANCHOR"))
        if i == 0 and opening:
            image = pathlib.Path(opening)
            strength, policy = 1.0, "openingReference"
